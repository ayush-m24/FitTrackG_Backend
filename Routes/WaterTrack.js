...
const { awardBadge } = require('../utils/badgeUtilities'); 
...
router.post('/addwaterentry', authTokenHandler, async (req, res) => {
    ...
    await user.save();
     ...
     // Award badge if the user's water intake exceeds the water goal
     const waterGoal = user.goalWater || 2000; // Default water goal if not set
     await awardBadge(userId, 'Water', amountInMilliliters);
