...
function checkAuth(req, res, next) {
    ...
    if (!authToken || !refreshToken) { //Details are not available send this reponse
        return res.status(401).json({ message: 'Authentication failed: No authToken or refreshToken provided' , ok : false });
    }

    //Check for authToken if details are avialable. User id is decoded. 
        ...
        if (err) {
            ...
            jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (refreshErr, refreshDecoded) => {
                if (refreshErr) {
                    ...
                    res.cookie('authToken', newAuthToken, { httpOnly: true, secure: false, sameSite: 'None', path: '/' });
                    res.cookie('refreshToken', newRefreshToken, { httpOnly: true, secure: false, sameSite: 'None', path: '/' });
